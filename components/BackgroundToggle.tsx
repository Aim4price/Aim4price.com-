...
export default function BackgroundToggle({ className, iconClassName }: { className?: string; iconClassName?: string }) {
  ...
  return (
    <button type="button" className={className} onClick={toggle} aria-label="Dark background" aria-pressed={dark} title={dark ? 'Switch to light background' : 'Switch to dark background'}>

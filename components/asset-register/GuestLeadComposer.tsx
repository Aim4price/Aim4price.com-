...
export default function GuestLeadComposer({assetIds,includePhotos,recipient,reports,ready,onChange,selectionKey}:{selectionKey:string;assetIds:string[];includePhotos:boolean;recipient?:{name:string;email:string;phone:string};reports:{label:string;file:File}[];ready:boolean;onChange:(url:string,email?:string)=>void}){
 const [fields,setFields]=useState({recipientName:recipient?.name||'',recipientEmail:recipient?.email||'',request:'',replyName:'',replyEmail:'',replyPhone:'',allowReply:true});
 const [history,setHistory]=useState<any[]>([]),[busy,setBusy]=useState(false),[notice,setNotice]=useState(''),[url,setUrl]=useState('');
 ...
 const r=await fetch('/api/asset-share-links/leads',{method:'POST',body:form}),d=await r.json();if(!r.ok)throw new Error(d.error||'Could not create the lead.');const next=`${window.location.origin}/asset-share/${d.share.token}`;setUrl(next);onChange(next,fields.recipientEmail);setNotice('Lead link ready. Preview it, then send it from your email or WhatsApp.');await load();
 ...
 <p>The basic card, photos you include and your request are visible to anyone with the link. Reports require the recipient’s confirmed email and active guest access, or an eligible verified Aim4price account. Reports are kept on the lead page, not attached to your email.</p>
 ...
 <label>Recipient email<input type="email" value={fields.recipientEmail} required maxLength={254} onChange={e=>update('recipientEmail',e.target.value)}/></label>
 ...
 <details><summary>Previously shared leads ({history.length})</summary>{history.map(lead=><div key={lead.token} className={styles.report}><span>{lead.recipient_name||lead.recipient_email} · {lead.recipient_email}<br/>{new Date(lead.created_at).toLocaleDateString('en-ZA')} {lead.revoked_at?'· Disabled':''}</span>{!lead.revoked_at&&<div className={styles.actions}><a href={`/asset-share/${lead.token}`} target="_blank" rel="noreferrer">View</a><button disabled={busy} type="button" onClick={()=>void revoke(lead.token)}>Disable</button></div>}</div>)}</details>
 </section>;
}

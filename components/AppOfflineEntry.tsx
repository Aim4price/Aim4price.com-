...
export default function AppOfflineEntry({ appRoot, worker }: { appRoot: string; worker: string }) {
  ...
  return <>{legacyCount > 0 ? <p role="status" style={{ margin: 0, padding: '12px 18px', background: 'var(--app-warning-soft, #fff4db)', color: 'var(--app-warning, #674c14)' }}>{legacyCount} older phone-saved update{legacyCount === 1 ? '' : 's'} need account verification before syncing. They remain on this phone. Keep this browser’s data until they have been reviewed.</p> : null}{offline ? <p role="status" style={{ margin: 0, padding: '12px 18px', background: 'var(--app-surface-soft, #e5f0e8)', color: 'var(--app-text, #163e2c)' }}>You’re offline. <a href={`${appRoot}/offline.html`}>Open saved work</a></p> : null}</>;
}

...
export default function DirectoryAdminAccess({onAdd}:{onAdd:(business:AdminBusiness)=>void}){
 ...
 {data.acceptances.map(a=><article key={a.id} className={styles.card}><strong>{a.business_name}</strong><span>{a.contact_name} · {a.email} {a.phone}</span><small>Accepted {new Date(a.accepted_at).toLocaleDateString('en-ZA')}</small>{a.business_id?<span>Listing already added · {a.listing_status}</span>:<button type="button" className={styles.button} onClick={()=>onAdd({id:'',name:a.business_name,email:a.email,status:'invited',details:{name:a.business_name,phone:a.phone}})}>Prepare listing</button>}</article>)}

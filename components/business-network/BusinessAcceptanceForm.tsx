...
export default function BusinessAcceptanceForm({businessName='',embedded=false}:{businessName?:string;embedded?:boolean}) {
 ...
 return <section className={`${styles.panel} ${embedded?'':styles.page}`}>
  <h2>Be part of the Aim4price directory</h2>
  ...
  {done ? <section className={styles.card} role="status"><h2>Thank you—your acceptance is recorded.</h2><p>We will review your details and manually prepare your listing. Submitting this form does not publish a listing or unlock shared reports.</p></section> : <form className={styles.card} onSubmit={async event=>{
   ...
   <label>Business name<input name="businessName" defaultValue={businessName} required minLength={2} maxLength={200}/></label>
   ...
   <p>An Aim4price account is a separate option. Nothing is charged by submitting this form.</p>

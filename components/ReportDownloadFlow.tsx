    ...
    <div ref={dialog} className={`${styles.surface} ${report.dialog}`} data-download-dialog="true" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-busy={busy} onKeyDown={event => {
      ...
      <div className={styles.body} data-download-body="true">
        {step === 'scope' ? <div data-download-grid="true">
          ...
          <div data-download-grid="true">{(['pdf','xlsx'] as const).map(format=><button key={format} type="button" disabled={busy} data-download-option="true" onClick={()=>void submit(format)}><span data-download-icon="true"><img src={format === 'pdf' ? '/brand/pdf.png' : '/brand/sheet.png'} alt="" /></span><span data-download-copy="true"><strong>{format === 'pdf' ? 'PDF report' : 'Excel workbook'}</strong><small>{format === 'pdf' ? 'Open a printable report.' : 'Download the selected records.'}</small></span></button>)}</div>
        </>}

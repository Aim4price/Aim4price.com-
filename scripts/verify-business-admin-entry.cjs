...
`);
  ...
  const page=await browser.newPage(),errors=[],saves=[];let searches=0,failSearch=false;
  ...
  page.on('request',req=>{
   ...
   if(u.pathname==='/api/business-network/google'){
    searches++;assert.equal(JSON.parse(req.postData()).query,'S Haddad George');
  ...
  const fill=async(label,value)=>{const h=await page.evaluateHandle(t=>[...document.querySelectorAll('label')].find(e=>e.firstChild?.textContent?.trim()===t)?.querySelector('input'),label);assert.ok(h.asElement(),`Missing ${label}`);await h.asElement().type(value);await h.dispose();};
  ...
  for(const width of [1440,430]){
   ...
   await fill('Business name and town','S Haddad George');await page.keyboard.press('Enter');await page.waitForFunction(()=>document.body.textContent.includes('George, South Africa'));
   await click('Link this business');await page.waitForFunction(()=>document.body.textContent.includes('Google profile linked'));
  ...
  assert.equal(searches,4);assert.equal(saves.length,4);assert.deepEqual(errors,[]);

...
(async () => {
  ...
  const options = { assetTitle: '2022 New Holland TT4.90 4WD Openstation', serialNumber: 'NH-TT490-2022-001309', qrImageUrl, logoUrl: customLogo, fallbackLogoUrl: fallback };
  ...
  try {
    ...
    const heading = await page.evaluate(() => {
      ...
      return { topAligned: Math.abs(logo.top - title.top) < 1, upper: title.top < label.top + 70, below: details.top >= logo.bottom, lines: document.querySelectorAll('.writingLines span').length };
    });
    assert.deepEqual(heading, { topAligned: true, upper: true, below: true, lines: 3 });
    console.log('PASS prominent artwork, top-aligned logo/title and lower writing lines without serial');
    await open();

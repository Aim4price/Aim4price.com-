...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      const allowed = ['/app-theme.css', '/app-offline/worker.js', '/app-offline/config.mjs', '/app-offline/store.mjs', '/app-offline/offline.mjs', '/app-offline/offline.css', '/field-manager/offline.html', '/field-manager/offline.css', '/field-manager/montserrat-latin.woff', '/field-manager/offline.mjs', '/field-manager/offline-store.mjs', '/field-manager-sw.js', '/app-push-worker.js'];
  ...
  let browser;
  try {
    browser = await puppeteer.launch({ executablePath: process.env.OFFLINE_TEST_CHROMIUM || process.env.FIELD_OFFLINE_BROWSER_PATH || await chromium.executablePath(), args: chromium.args, headless: true, pipe: true });

...
(async () => {
  ...
  try {
    ...
    const waitScale=expected=>page.waitForFunction(value=>Math.abs(Number(document.querySelector('[data-website-canvas]')?.dataset.websiteScale)-value)<.001,{},expected);
    await waitScale(1);
    ...
    await page.setViewport({width:844,height:390,isMobile:false,hasTouch:true});

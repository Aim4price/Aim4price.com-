/** Scaled website canvases can round scroll positions by a rendered pixel. */
export function getHeaderNavScrollTolerance(element: HTMLElement) {
  const width = element.getBoundingClientRect().width;
  return Math.max(1, width ? element.offsetWidth / width : 1) + 1;
}

/** Snap to the nearest item, or move an arrow one item from the current stop. */
export function snapHeaderNav(element: HTMLElement, direction: -1 | 0 | 1 = 0) {
  const max = Math.max(0, element.scrollWidth - element.clientWidth);
  const tolerance = getHeaderNavScrollTolerance(element);
  const positions = Array.from(new Set([
    0,
    ...Array.from(element.children, (link) => (link as HTMLElement).offsetLeft)
      .filter((position) => position < max - tolerance),
    max,
  ]));
  const nearest = positions.reduce((best, position, index) =>
    Math.abs(position - element.scrollLeft) < Math.abs(positions[best] - element.scrollLeft) ? index : best, 0);
  const index = Math.max(0, Math.min(positions.length - 1, nearest + direction));
  element.scrollTo({

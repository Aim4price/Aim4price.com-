...
export type AdminAssetOutcome = 'sold' | 'traded_in' | 'scrapped';

export type AdminAssetOutcomeRow = {
  id: string;
  assetId: string;
  assetTitle: string;
...
export type AdminAssetOutcomesReport = {
  generatedAtIso: string;
  metrics: {
    totalOutcomes: number;
    totalSold: number;
    totalTradedIn: number;
    totalScrapped: number;
    helpedByAim4price: number;
    soldHelpedByAim4price: number;
    tradedInHelpedByAim4price: number;
    scrappedHelpedByAim4price: number;
...
function influence(value: unknown): AdminAssetOutcomeRow['aim4priceInfluence'] {
  const normalized = text(value).toLowerCase();
  return normalized === 'yes' || normalized === 'no' || normalized === 'unsure' ? normalized : 'unknown';
}

function transferStatus(row: OutcomeRow): AdminAssetOutcomeRow['transferStatus'] {
...
function outcome(value: unknown): AdminAssetOutcome {
  const normalized = text(value).toLowerCase();
  if (normalized === 'traded_in' || normalized === 'scrapped') return normalized;
  return 'sold';
}

export async function getAdminAssetOutcomesReport(): Promise<AdminAssetOutcomesReport> {
  await ensureAssetTransferSchema();
  const result = await getDb().query<OutcomeRow>(
    `select
       event.id::text,
     ...
     where event.event_type = 'disposed'
       and event.reason in ('sold', 'traded_in', 'scrapped')
     order by event.effective_date desc, event.created_at desc
     limit 5000`,
  );

  const outcomes = result.rows.map<AdminAssetOutcomeRow>((row) => {
    ...
    return {
      ...
      sourceUserId: text(row.seller_user_id || row.original_owner_user_id || row.owner_user_id),
      sourceName: text(row.profile_business_name || row.profile_display_name || row.actor_organisation || row.actor_name) || 'Unknown account',
      outcome: outcome(row.reason),
      outcomeDate: row.effective_date,
  ...
  const helpedByAim4price = outcomes.filter((item) => item.aim4priceInfluence === 'yes').length;
  const notHelpedByAim4price = outcomes.filter((item) => item.aim4priceInfluence === 'no').length;
  const influenceUnknown = outcomes.length - helpedByAim4price - notHelpedByAim4price;
  ...
  return {
    ...
    metrics: {
      ...
      totalSold: outcomes.filter((item) => item.outcome === 'sold').length,
      totalTradedIn: outcomes.filter((item) => item.outcome === 'traded_in').length,
      totalScrapped: outcomes.filter((item) => item.outcome === 'scrapped').length,
      helpedByAim4price,
      soldHelpedByAim4price: outcomes.filter((item) => item.outcome === 'sold' && item.aim4priceInfluence === 'yes').length,
      tradedInHelpedByAim4price: outcomes.filter((item) => item.outcome === 'traded_in' && item.aim4priceInfluence === 'yes').length,
      scrappedHelpedByAim4price: outcomes.filter((item) => item.outcome === 'scrapped' && item.aim4priceInfluence === 'yes').length,
      notHelpedByAim4price,
      influenceUnknown,
      helpRatePercent: answered ? Math.round((helpedByAim4price / answered) * 1000) / 10 : 0,
      transferredAccounts: outcomes.filter((item) => item.transferStatus === 'claimed').length,
      pendingTransfers: outcomes.filter((item) => item.transferStatus === 'pending' || item.transferStatus === 'expired').length,
    },
    outcomes,
  };
}

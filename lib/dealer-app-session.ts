...
export type DealerAppSession = { kind:'dealer-staff'; staffId:string; dealerUserId:string; displayName:string; username:string; version:number };
export async function getDealerAppSession(): Promise<DealerAppSession|null> {
  ...
  return {kind:'dealer-staff',staffId:p.staffId,dealerUserId:p.dealerUserId,displayName:row.display_name,username:row.username,version:p.version};
}

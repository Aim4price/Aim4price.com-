...
export type PushEvent = { id: string; category: PushCategory; title: string; body: string; href: string; createdAtIso: string; reminder?: boolean };
export async function listPushEvents(who: PushIdentity, access: NonNullable<Awaited<ReturnType<typeof resolvePushAccess>>>): Promise<PushEvent[]> {
  ...
  if (who.app === 'owner') {
    ...
    if (access.admin) {
      ...
      for (const item of current) {
        ...
        const category = ['lead','asset_discovery','marketplace_sourcing'].includes(item.category) ? 'enquiries'
          : ['dealer_correction','dealer_schedule','dealer_cost'].includes(item.category) ? 'approvals' : null;
    ...
    if (who.app === 'dealer' && access.categories.includes('maintenance')) {
      ...
      for (const item of notifications.filter(item => !item.isRead)) events.push({ ...item,
        category: item.status === 'assigned_problem' ? 'assignments' : 'maintenance',
        href: root + '/maintenance', reminder: item.status !== 'assigned_problem' });
    }
    if (access.categories.includes('enquiries')) {
      ...
      for (const item of current) {
        ...
        if (who.app === 'middleman' && item.category === 'lead') continue;
        events.push({ ...item, category: 'enquiries', href: root + (item.category === 'asset_discovery' ? '/discovery' : item.category === 'lead' ? '/leads' : '/marketplace') });
      }
    }
  }
  return events.filter(event => access.categories.includes(event.category));
}

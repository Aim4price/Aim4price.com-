...
export async function createDealerStaff(dealerUserId: string, input: Record<string, unknown>) {
  ...
  try {
    const r = await getDb().query<Row>(`insert into dealer_app_staff(dealer_user_id,display_name,username,username_normalized,password_hash,is_active)
      values($1,$2,$3,$4,$5,true) returning *`, [dealerUserId, displayName, username, username, await hashPassword(password)]);
    return map(r.rows[0]);
...
export async function updateDealerStaff(dealerUserId: string, id: string, input: Record<string, unknown>) {
  ...
  const bump = passwordChanged || active !== row.is_active;
  try {
    const r = await getDb().query<Row>(`update dealer_app_staff set display_name=$3, username=$4, username_normalized=$4,
      password_hash=$5, is_active=$6, session_version=session_version+$7, updated_at=now()
      where id=$1 and dealer_user_id=$2 returning *`, [id,dealerUserId,displayName,username,passwordHash,active,bump?1:0]);
    return map(r.rows[0]);
  } catch (e: any) { if (e?.code === '23505') throw new Error('That username is already in use.'); throw e; }
}

...
export default async function AccountantSharedCostsPage({ params }: { params: { shareId: string } }) {
  ...
  try {
    await getAccountantLedger({ accountantUserId: session.user.id, shareId: params.shareId, kind: 'cost' });
  } catch {
    redirect(`/accountant/registers/${encodeURIComponent(params.shareId)}`);
  }

  return <MyInvoicesClient accountantShareId={params.shareId} />;
}

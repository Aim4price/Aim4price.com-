...
export default async function DealerShowroomPage() {
  ...
  if (dealerAppSession && !dealerRoleCan(dealerAppSession.role, 'showroom')) redirect('/middleman');
  let showroom;
  try {
  ...
  return (
    <div className={`${styles.module} ${styles.compactWorkspace}`}>
      <MiddlemanShowroomManager
        ...
        advertDesignHref={!dealerAppSession || dealerRoleCan(dealerAppSession.role, 'ad_studio') ? '/middleman/ad-studio' : null}
      />
    </div>
  );
}

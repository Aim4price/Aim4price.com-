...
export default function AssetTransfersClient() {
  ...
  async function post(body: Record<string, unknown>): Promise<TransferResponse> {
    ...
    const payload = await response.json().catch(() => null) as TransferResponse | null;
    if (response.status === 401) { window.location.assign('/auth#login'); throw new Error('You must be signed in.'); }
  ...
  async function claim(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setAssetIdentifier('');
      setTransferCode('');
      setNotice({ tone: 'success', message: `${payload.claimed.assetTitle} is now in your selected asset register.` });
    } catch (error) {
  ...
  async function cancel(transferId: string) {
    ...
    try {
      ...
      setNotice({ tone: 'success', message: 'The transfer was cancelled and the sold asset was archived.' });
    } catch (error) {
    ...
    <main className={styles.page}>
      ...
      <section className={styles.shell}>
        <header className={styles.hero}>
          <div><p>Account</p><h1>Asset transfers</h1><span>Claim an asset you bought, or manage assets waiting for a buyer.</span></div>
        ...
        <section className={styles.outgoingSection} aria-labelledby="outgoing-transfers-title">
          ...
          {loading ? <div className={styles.emptyState}>Loading transfers…</div> : outgoing.length ? <div className={styles.transferList}>{outgoing.map((item) => (
            <article key={item.id} className={styles.transferCard}>
              <div className={styles.transferTitle}><div><strong>{item.assetTitle}</strong><span>{item.assetIdentifierLabel}: {item.assetIdentifier}</span></div><span className={`${styles.statusBadge} ${styles[`status_${item.status}`]}`}>{STATUS_LABELS[item.status]}</span></div>
          ...
          ))}</div> : <div className={styles.emptyState}><strong>No sent assets yet</strong><span>When you sell an asset, choose “Send to buyer” in the asset removal flow.</span></div>}
        </section>
      </section>
      ...
      {receipt ? <div className={styles.modalBackdrop} role="dialog" aria-modal="true" aria-labelledby="new-transfer-code-title" onClick={() => setReceipt(null)}>
        <section className={styles.modalCard} onClick={(event) => event.stopPropagation()}>
          <header><div><h2 id="new-transfer-code-title">New transfer code</h2><p>Share the identifier and code with the buyer. The previous code no longer works.</p></div><button type="button" onClick={() => setReceipt(null)} aria-label="Close transfer code">×</button></header>

...
export default function OwnerAppLoginClient({ hasAccountSession = false }: { hasAccountSession?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (cleanUsername.length < 3 || !password) { setError('Enter your Owner App username and password.'); return; }
    setBusy(true); setError('');
    try {
    ...
    <main className={styles.loginPage}>
      <section className={styles.loginCard} aria-labelledby="owner-login-title">
        <header className={styles.loginHeader}>
          ...
          <p className={styles.loginIntro}>Use the Owner App username and password supplied by the account owner.</p>
        </header>
        {accountSessionActive ? (
          ...
          <form className={styles.loginForm} onSubmit={submit}>
            ...
            <div className={styles.field}><label className={styles.fieldLabel} htmlFor="owner-app-password">Password</label><span className={styles.inputWrap}><input id="owner-app-password" className={styles.passwordInput} type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" disabled={busy} required /><button type="button" className={styles.passwordToggle} onClick={() => setShowPassword((current) => !current)} disabled={busy}>{showPassword ? 'Hide' : 'Show'}</button></span></div>
            {error ? <p className={styles.error}>{error}</p> : null}

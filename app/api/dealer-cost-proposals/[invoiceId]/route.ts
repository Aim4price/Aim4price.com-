...
function normalizeDecision(value: unknown): DealerCostOwnerDecision | null {
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const invoice = await getPendingOwnerDealerCost(
      ownerUserId,
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (!decision) {
    return NextResponse.json(
      { ok: false, error: 'Choose whether this cost must be stored in your Cost Ledger.' },
      { status: 400 },
    );
  }
  ...
  try {
    const ownerStorageStatus = await resolveDealerCostOwnerDecision({
      ownerUserId,
      invoiceId: String(context.params.invoiceId ?? '').trim(),
      decision,
    });
    return NextResponse.json({
      ok: true,

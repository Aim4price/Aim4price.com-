...
export async function GET(request: NextRequest) {
  ...
  try { return NextResponse.json(await listOwnerAppOverview(access.ownerUserId, range(request.nextUrl.searchParams.get('range')))); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await dismissOwnerAppOverviewItem(access.ownerUserId, range(body.range), String(body.itemId ?? ''), String(body.sourceId ?? ''));

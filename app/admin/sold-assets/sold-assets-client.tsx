...
export default function SoldAssetsClient({ report, allocationAccounts }: { report: AdminAssetSalesReport; allocationAccounts: AdminAssetAllocationAccount[] }) {
  ...
  async function submitAction() {
    ...
    if (actionDialog.mode === 'allocate' && !selectedBuyerUserId) {
      setActionError('Choose the owner account that should receive this asset.');
      return;
    }
    setActionBusy(true);
    setActionError('');
    ...
    {actionDialog ? <div className={styles.actionDialog} role="dialog" aria-modal="true" aria-labelledby="sold-asset-action-title">
      ...
      <section className={styles.actionModal}>
        ...
        {actionDialog.mode === 'allocate' ? <div className={styles.actionBody}>
          <div className={styles.actionNotice}><strong>Ownership allocation</strong><p>The portable asset dossier moves to the selected active owner account. Seller-private invoices, finance, insurance and account access do not move.</p></div>
          <label className={styles.accountSearch}><span>Find owner account</span><input type="search" value={accountSearch} onChange={(event) => setAccountSearch(event.target.value)} placeholder="Business, person or email" autoFocus /></label>
          <div className={styles.accountList} role="radiogroup" aria-label="Choose destination owner account">{availableAccounts.length ? availableAccounts.map((account) => <button key={account.userId} type="button" className={selectedBuyerUserId === account.userId ? styles.accountSelected : ''} role="radio" aria-checked={selectedBuyerUserId === account.userId} onClick={() => setSelectedBuyerUserId(account.userId)}><span><strong>{account.name}</strong><small>{account.email || 'No email shown'}</small></span><em>{account.accountSubtype}</em></button>) : <p>No active owner accounts match this search.</p>}</div>

...
Desktop Account → Notifications opens a modal on /account using the same shell, backdrop, scrolling and footer as Marketplace contact. Changes are saved with Save notifications; Cancel discards them. Legacy /account/notifications links open this modal. Desktop and app settings use red/off and green/on switches with a sliding thumb. Its master and category switches control phone alerts across the signed-in account, using website credentials only. App members cannot change these account-wide settings. Owner, Dealer and Middleman apps have a gear beside the Notifications heading; it opens a modal for personal category preferences and this phone’s permission/registration. An account restriction takes precedence over personal preferences without overwriting them. Restoring account alerts does not re-enable an unsubscribed phone or override a member’s own off switch. The normal inboxes are not muted by turning off phone alerts.
...
Database tables and a stable VAPID key pair are created automatically using the project's existing schema-readiness approach. Private keys stay server-side in app_push_keys. Account policy lives in account_push_preferences; app login preferences stay in app_push_preferences. Back up this table with the database; replacing the key requires re-enabling notifications on devices. The application database role needs CREATE TABLE permission for the first start. No keys or push endpoints should be logged.
...
Run npm run test:phone-notifications, npm run test:notifications and npm run typecheck. Verify Desktop → Account → Notifications saves across reload, account switches stop phone alerts, and app settings open/close without navigating. Verify an installed iPhone (iOS 16.4+) and Android device: enable, test, close the app, trigger a new event, tap it, disable, sign out, switch accounts, revoke staff access. Confirm each app opens its own page and no old account text appears. Check database DDL and production instrumentation startup in staging before rollout.

...
test('logout clears only the selected app cookies', async () => {
  for (const realm of ['dealer', 'middleman']) {
    ...
    const { POST } = await load('app/api/dealer/logout/route.ts', {
      ...
      '../../../../lib/notification-request-origin': await load('lib/notification-request-origin.ts'),
    ...
    await POST({ url: 'http://internal-railway:3000/api/' + realm + '/logout', headers: new Headers({ origin: 'https://www.aim4price.com' }) });

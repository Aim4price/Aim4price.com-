...
test("Marketplace contact follows existing authorization boundaries", () => {
  ...
  assert.match(marketplaceWhatsAppAction, /Chat on WhatsApp/);
  assert.match(marketplaceWhatsAppAction, /Message \{contactName\} about this advert/);
  assert.match(marketplaceWhatsAppAction, /className=\{styles\.description\}/);
  assert.doesNotMatch(marketplaceWhatsAppAction, /<small>/);
  assert.match(marketplaceWhatsAppAction, /styles\.openCue/);
  ...
  assert.match(marketplaceWhatsAppStyles, /\.description\s*\{[^}]*color:\s*#f1fff9;[^}]*font-size:\s*0\.76rem/);
  assert.match(marketplaceWhatsAppStyles, /\.openCue\s*\{[^}]*display:\s*inline-flex;[^}]*padding:\s*0 0\.72rem/);
